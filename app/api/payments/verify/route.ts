...
// Create clients for Celo chains only
const clients = {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const cusdAddress = CUSD_ADDRESSES[chainId as keyof typeof CUSD_ADDRESSES];
    if (!cusdAddress) {
      return NextResponse.json(
        { error: 'cUSD not supported on this chain' },
        { status: 400 }
      );
    }

    // Expected amounts for cUSD (18 decimals)
    const expectedAmount = paymentType === PaymentType.DAILY_ACCESS 
