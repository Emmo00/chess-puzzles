...
export function FrontendErrorsPanel({ adminAccessKey }: { adminAccessKey: string }) {
  ...
  return (
    <section className="bg-white border-4 border-black p-4 shadow-[8px_8px_0px_#000000]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
        ...
        <div className="flex gap-2 mt-2 sm:mt-0">
          <select
            ...
            className="border-2 border-black px-2 py-1 text-xs font-black bg-yellow-100"
          >
          ...
          <button
            ...
            className="bg-cyan-300 text-black px-3 py-1 font-black text-xs uppercase border-2 border-black shadow-[3px_3px_0px_#000000]"
          >
            Refresh
          </button>
        </div>
      ...
      {errorMsg && (
        <div className="bg-red-300 border-2 border-black p-2 text-xs font-black uppercase text-black mb-4 shadow-[4px_4px_0px_#000000]">
          <div>{errorMsg}</div>
          <TelegramSupportLink />
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="text-xs font-black uppercase p-4 text-center bg-yellow-100 border-2 border-black shadow-[3px_3px_0px_#000000]">No errors found.</div>
      ) : (
        <div className="space-y-4">
          {errors.map((err) => (
            <div key={err._id} className="border-2 border-black p-3 text-sm space-y-2 bg-white shadow-[4px_4px_0px_#000000]">
              <div className="flex justify-between items-start">
                <span className="font-bold text-red-600 break-all">{err.message}</span>
                {err.status === "new" && (
                  <button
                    onClick={() => handleResolve(err._id)}
                    className="bg-green-400 border-2 border-black px-2 py-1 text-[10px] font-black uppercase hover:bg-green-500 whitespace-nowrap ml-2 shadow-[2px_2px_0px_#000000]"
                  >
                    Resolve
                  </button>
                )}
              </div>
              <div className="grid grid-cols-2 gap-2 text-xs font-mono bg-yellow-50 p-2 border-2 border-black">
